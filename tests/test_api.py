...
from pyzaptec.api import Charger, Installation, Zaptec, ZaptecBase
from pyzaptec.const import API_RETRIES
from pyzaptec.exceptions import (
    AuthenticationError,
...
from pyzaptec.redact import Redactor
from pyzaptec.zconst import ZCONST
...
async def test_request_401_refreshes_token_then_succeeds(
    ...
    zap, _ = _make_zaptec(
        [
            ...
            FakeResponse(HTTPStatus.OK, json_data=payload),
        ]
    )
    refresh = AsyncMock()
...
def test_stream_update_routes_to_matching_charger(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_command_posts_to_send_command_url(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_poll_info_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("pyzaptec.api.validate", Mock())
...
@pytest.mark.asyncio
async def test_installation_poll_info_strips_logo(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
